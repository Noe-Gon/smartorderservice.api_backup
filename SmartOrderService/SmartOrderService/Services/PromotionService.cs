    ...
    public class PromotionService
    {
        ...
        public PromotionResult GetSalePromotions(int UserId, int InventaryId)
        {
            ...
            using (var context = db)
            {
                ...
                lstPromotionTypeCatalogDto = db.so_promotion_type_catalog.Where(x => x.status).Select(x => new PromotionTypeCatalog()
                {
                    code = x.code,
                    id = x.id,
                    name = x.name
        ...
        public PromotionSaleDeliveryResult GetSalePromotionsDelivered(int UserId, int InventaryId)
        {
            List<RelSalePromotion> lstRelSalePromotion = new List<RelSalePromotion>();
            List<PromotionSaleDetailGiftArticle> lstPromotionSaleDetailGiftArticle = new List<PromotionSaleDetailGiftArticle>();
            List<PromotionSaleDetailGiftProduct> lstPromotionSaleDetailGiftProduct = new List<PromotionSaleDetailGiftProduct>();
            List<PromotionSaleDetailPromotionProduct> lstPromotionSaleDetailPromotionProduct = new List<PromotionSaleDetailPromotionProduct>();
            List<SaleWithPromotion> lstSaleWithPromotion = new List<SaleWithPromotion>();
            PromotionSaleDeliveryResult promotionSaleDeliveryResult = new PromotionSaleDeliveryResult();

            using (var context = db)
            {
                SqlParameter userId = new SqlParameter("@UserId", UserId);
                SqlParameter inventaryId = new SqlParameter("@InventaryId", InventaryId);
                SqlParameter optionId = new SqlParameter("@OptionId", 6);

                //Recuperando el catálogo de articulos
                lstSaleWithPromotion = context.Database.SqlQuery<SaleWithPromotion>("sp_getPromotions @UserId, @InventaryId, @OptionId", userId, inventaryId, optionId).ToList();

                //Recuperando la configuración de las promociones
                userId = new SqlParameter("@UserId2", UserId);
                inventaryId = new SqlParameter("@InventaryId2", InventaryId);
                optionId = new SqlParameter("@OptionId2", 7);
                lstRelSalePromotion = context.Database.SqlQuery<RelSalePromotion>("sp_getPromotions @UserId2, @InventaryId2, @OptionId2", userId, inventaryId, optionId).ToList();

                //Recuperando los productos a vender para la promoción
                userId = new SqlParameter("@UserId3", UserId);
                inventaryId = new SqlParameter("@InventaryId3", InventaryId);
                optionId = new SqlParameter("@OptionId3", 8);
                lstPromotionSaleDetailPromotionProduct = context.Database.SqlQuery<PromotionSaleDetailPromotionProduct>("sp_getPromotions @UserId3, @InventaryId3, @OptionId3", userId, inventaryId, optionId).ToList();

                //Recuperando los productos de regalo de la promoción
                userId = new SqlParameter("@UserId4", UserId);
                inventaryId = new SqlParameter("@InventaryId4", InventaryId);
                optionId = new SqlParameter("@OptionId4", 9);
                lstPromotionSaleDetailGiftProduct = context.Database.SqlQuery<PromotionSaleDetailGiftProduct>("sp_getPromotions @UserId4, @InventaryId4, @OptionId4", userId, inventaryId, optionId).ToList();
            ...
            promotionSaleDeliveryResult.SaleWithPromotion = lstSaleWithPromotion;
            promotionSaleDeliveryResult.RelSalePromotion = lstRelSalePromotion;
            promotionSaleDeliveryResult.PromotionSaleDetailPromotionProduct = lstPromotionSaleDetailPromotionProduct;
            promotionSaleDeliveryResult.PromotionSaleDetailGiftProduct = lstPromotionSaleDetailGiftProduct;
            promotionSaleDeliveryResult.PromotionSaleDetailGiftArticle = lstPromotionSaleDetailGiftArticle;

            return promotionSaleDeliveryResult;
        }
    }
}